...
// ─── FFmpeg: download once, instantiate fresh for each exec() ─────────────────
// ffmpeg-core's state is single-use — after exec() the WASM instance is spent.
// Solution: cache the JS text + WASM binary, create a new instance per clip.
...
// Create a brand-new core instance. Must be called for EVERY exec().
...
export default function ClipEditor({ events, playerRef, onUpdateClip, onEditClip }: ClipEditorProps) {
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      // Load ffmpeg assets if not already cached
      ...
      for (let i = 0; i < clipsToExport.length; i++) {
        ...
        if (ret !== 0) throw new Error(`FFmpeg error en clip ${i + 1} (código ${ret})`);

        const data: Uint8Array = core.FS.readFile(outName);
        // Copy data out before the core goes out of scope
        const copyBuf = new Uint8Array(data).buffer as ArrayBuffer;
        const dlBlob = new Blob([copyBuf], { type: "video/mp4" });

        // Sequential download with delay — browsers need this for multi-file
        await new Promise<void>((resolve) => {
          ...
          // Wait before triggering next download and before revoking URL
          setTimeout(() => {
            URL.revokeObjectURL(url);
            resolve();
          }, 1500);
