...
export function usePartidos() {
  ...
  const crearPartido = useCallback((
    ...
    const p: Partido = {
      ...
      score: { local: 0, visitante: 0 }, events: [], players: [], annotations: [], createdAt: Date.now(),
    };
  ...
  const addAnnotation = useCallback((partidoId: string, a: Annotation) =>
    setPartidos(prev => prev.map(p =>
      p.id !== partidoId ? p : { ...p, annotations: [...(p.annotations ?? []), a] }
    )), []);

  const deleteAnnotation = useCallback((partidoId: string, annotationId: string) =>
    setPartidos(prev => prev.map(p =>
      p.id !== partidoId ? p : { ...p, annotations: (p.annotations ?? []).filter(a => a.id !== annotationId) }
    )), []);

  const updateAnnotation = useCallback((partidoId: string, annotationId: string, patch: Partial<Annotation>) =>
    setPartidos(prev => prev.map(p =>
      p.id !== partidoId ? p : {
        ...p, annotations: (p.annotations ?? []).map(a => a.id === annotationId ? { ...a, ...patch } : a)
      }
    )), []);
